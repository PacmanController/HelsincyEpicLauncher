...
- 当前正式登录链路使用系统浏览器打开 Epic 登录页，由用户把 `authorizationCode` 优先粘贴回应用；若浏览器返回的是完整 JSON 响应或 `redirectUrl`，也由 Auth 模块在内部提取授权码并完成 token 交换
...
4. 用户在浏览器中完成登录，拿到 `authorizationCode`、完整 JSON 响应或 `redirectUrl`
